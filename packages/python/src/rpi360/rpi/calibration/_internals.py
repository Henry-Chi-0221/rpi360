...
def _as_calibration_points(
    ...
    for object_set, image_set in zip(object_points, image_points, strict=False):
...
def _reprojection_errors(
    ...
    errors = []
    for objects, observed, rvec, tvec in zip(object_points, image_points, rvecs, tvecs, strict=False):
...
def _draw_intrinsic_preview(
    ...
    *,
    ...
    for index, (line, color) in enumerate(zip(lines, colors, strict=False)):
        cv2.putText(
            panel,
            line,
            (18, 29 + index * 31),
