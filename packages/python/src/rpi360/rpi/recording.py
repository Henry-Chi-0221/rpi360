...
def mux_recording(
    ...
    *,
    ...
    command = [require_executable("ffmpeg"), "-v", "error", "-y", "-copyts"]
    for track, offset in zip(track_paths, offsets_us, strict=True):
        if offset:
            command.extend(["-itsoffset", str(offset / 1_000_000)])
...
def recording_directory() -> Path:
    root = Path.home() / ".local/share/rpi360/legacy-recovery"
    root.mkdir(parents=True, exist_ok=True)
    return Path(tempfile.mkdtemp(prefix="rpi360-recording-", dir=root))
